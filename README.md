...
### WezTerm-style embedded experience

The integrated terminal now ships with a WezTerm-style dark preset by default.
It is designed for AI/TUI workflows (for example Codex CLI):

- Dark, high-contrast, compact terminal look.
- Larger default font and tighter line height.
- Large scrollback buffer for traceable output history.
- Right click defaults to copy/paste workflow.
- Windows integrated defaults use `powershell -NoLogo` and keep `useWin32Conhost` off.

You can tune these quickly from plugin settings:

1. Open plugin settings.
2. Go to `Embedded terminal experience`.
3. Configure:
   1. `Theme preset` (WezTerm-style dark / follow Obsidian)
   2. `Font family`
   3. `Font size`
   4. `Line height`
   5. `Scrollback buffer`
   6. `Right click behavior`
   7. `Default shell` (PowerShell, pwsh, cmd, Git Bash, WSL, etc. by platform)

Per-profile controls are also available in the profile modal:

- `Theme preset`
- `Right click action`
- `Terminal options` (raw xterm options)

### Acceptance steps

Use these steps in Obsidian to validate appearance, functionality, and Codex compatibility.

1. Open a new integrated terminal from ribbon/command palette.
2. Verify appearance:
   1. Dark gradient panel, rounded border, subtle shadow.
   2. Larger font and compact line spacing.
   3. Scrollbar is visible and usable.
3. Verify multi-instance:
   1. Open at least 2 integrated terminals.
   2. Confirm both run independently in tabs/splits.
4. Verify copy behavior:
   1. Select text with mouse.
   2. Right click once to copy selection.
   3. Right click with no selection to paste clipboard.
5. Verify long-output traceability:
   1. Run a verbose command (for example `git log --oneline --decorate --graph`).
   2. Scroll back and confirm earlier lines are still available.
6. Verify Codex/TUI interaction:
   1. Start `codex`.
   2. Confirm startup layout is stable (no repeated initial blocks).
   3. Type `/`, Chinese text, backspace, and Enter.
   4. Continue for two rounds and confirm cursor/input behavior remains correct.
